...
cons_t *sgmt_read(char *line, int *pos)
{
	...
	{
		int len1 = 0, len2 = 0;
		while (line[*pos + len1] != ' ' && line[*pos + len1] != ')')
		len1++;
		while (isdigit(line[*pos + len2]))
		len2++;
		if (len1 == len2) {
	...
	//-----------Parse'setq'-----------
	case 's':
	if (line[*pos + 1] == 'e' && line[*pos + 2] == 't'
		&& line[*pos + 3] == 'q' && line[*pos + 4] == ' ') {
		cell->type = SETQ;
		(*pos) = (*pos) + 4;
	} else {
		set_symbol(line, pos, cell);
	...
	//------------Parse'defun'---------
	case 'd':{
		if (line[*pos + 1] == 'e' && line[*pos + 2] == 'f'
		&& line[*pos + 3] == 'u' && line[*pos + 4] == 'n'
		&& line[*pos + 5] == ' ') {
		cell->type = DEFUN;
		(*pos) = (*pos) + 5;
		} else {
